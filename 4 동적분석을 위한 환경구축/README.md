...
이전 문서에서는 실제 기기(Jooan C9TS IP 카메라)에서 추출한 펌웨어를 대상으로 Binwalk를 활용해 내부 구조를 분석하고 루트 파일 시스템을 확인하며 부팅 흐름을 정리했다. 본 문서에서는 해당 분석 결과를 바탕으로 QEMU 기반의 동적 분석 환경 구축 절차와 분석 방법을 제시한다. 

먼저 Buildroot를 활용하여 Linux 커널 이미지를 생성하는 방법을 소개하고, 이어 QEMU 에뮬레이팅에 사용할 루트 파일 시스템 구성 과정을 정리한다. 마지막으로 QEMU 환경에서 gdbserver를 사용해 원격 디버깅을 수행하며 펌웨어 동작을 실시간으로 추적·분석하는 방법을 제시한다.

## 4.1 Builldroot를 활용한 커널 이미지 생성

### **✅ 준비사항**

- [ ]  펌웨어의 아키텍처 및 엔디언을 확인했는가?
- [ ]  펌웨어에 사용된 커널 버전을 확인했는가?
...
정적 분석 결과, 기기에서 추출한 펌웨어는 MIPS 32비트 Little Endian 아키텍처를 사용중이며, 커널 버전은 Linux 3.10.14로 확인되었다. 에뮬레이션 과정에서 버전 불일치로 인한 호환성 문제를 방지하기 위해 Buildroot로 동일 버전의 커널 이미지를 생성하여 환경을 구성한다.
...
다음의 [`Dockerfile`](../src/Dockerfile)을 이용한다.
...
open함수를 후킹하기 위한 [`코드`](../src/hook.c)를 작성한다.

이 코드는 jooanipc 프로세스가 /dev/motor 파일을 열려고 시도할 때 발생하는 오류를 우회하기
위해 open 함수를 후킹하는 역할을 한다.

먼저 dlsym 함수와 RTLD_NEXT 플래그를 사용하여 현재 라이브러리(hook.so) 다음 순서에
